...
function ContactMap(props) {
  ...
  const handleClose = () => {
    ...
    const templateParams = {
      ...
      message: values.email+ " "+values.phone+" "+values.message,
    };

    try {
      const result = await emailjs.send(
        "service_uid5otg", // Remplace par ton service ID
        'template_z6dscos', // Remplace par ton template ID
        templateParams,
        't2jyWL1E2-LLpGVjT' // Remplace par ta clé publique
      );
      setNotif(true)
      setValues({
    name: '',
      ...
      <Container>
        <div className={classes.pageWrap}>
          <Snackbar
            ...
            message={<span id="message-id">{t('form_success')}</span>}
          />
                      ...
                      <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                          <TextValidator
                            ...
                            value={values.email}
                            validators={['isEmail']}
                            errorMessages={[t('form_required2')]}
                          />
                        </Grid>
                        <Grid item xs={12}>
                          <TextValidator
